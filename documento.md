...
| **v20** ✅ | **Turbo Shadow-Build Strategy e Correção Final do Z-Order**: Implementada construção em lote usando um `tempComp` invisível no `SplitGroup.jsx` para contornar a extrema lentidão da UI do AE durante a adição sequencial de múltiplos shape groups. Toda a hierarquia é estruturada off-timeline e transportada em milissegundos usando uma única operação `copyToComp()`. Paralelamente, **o bug de Z-Order persistente da v18 foi expurgado**: No `SplitLayer`, o loop do `copyToComp` foi corrigido para `1 → numLayers` (forwards) garantindo a Frente no topo da timeline. No `SplitGroup`, a estratégia falha de usar `moveTo(1)` foi inteiramente abolida. Sem o `moveTo`, o `addProperty` do AE enfileira os elementos na ordem natural de processamento (Fundo no Index 1, Frente no Index N). Como as Shape Layers renderizam de baixo para cima visualmente (Index N por último = na frente), a ordem original do Illustrator é restaurada com precisão perfeita. |
